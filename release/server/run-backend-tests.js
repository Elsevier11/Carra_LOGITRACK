...
async function runArticleServiceTests() {
    ...
    assert.equal(buildPositionLabel('VASCA', 'A', 12.345, 1), 'A @ 12.35m (L1)');
    ...
    const overlapError = validateArticleTransition(
        ...
        { id: 'v1', tipo: 'VASCA', stato: 'IN_AREA', fila: 'A', offsetInizio: 5, lunghezza: 4 },
        [{ id: 'v2', stato: 'IN_AREA', fila: 'A', offset_inizio: 7, lunghezza: 5 }]
    );
    ...
    try {
        await run(
            ...
            `INSERT INTO articoli (id, codice, cliente, commessa, lunghezza, posizione, fila, offset_inizio, tipo, livello, colore, stato, data_creazione)
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
            ['s-low', 'S-LOW', 'Cliente', 'Comm', 1, 'P1 @ Pila (L1)', 'P1', 0, 'SOLETTA', 1, '#111111', 'IN_AREA', '2026-01-01T00:00:00.000Z']
        );
        await run(
            db,
            `INSERT INTO articoli (id, codice, cliente, commessa, lunghezza, posizione, fila, offset_inizio, tipo, livello, colore, stato, data_creazione)
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
            ['s-top', 'S-TOP', 'Cliente', 'Comm', 1, 'P1 @ Pila (L3)', 'P1', 0, 'SOLETTA', 3, '#222222', 'IN_AREA', '2026-01-02T00:00:00.000Z']
        );
    ...
    try {
        await run(
            ...
            `INSERT INTO articoli (id, codice, cliente, commessa, lunghezza, posizione, fila, offset_inizio, tipo, livello, colore, stato, data_creazione)
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
            ['s1', 'S1', 'Cliente', 'Comm', 1, null, null, null, 'SOLETTA', 1, '#111111', 'CREATA', '2026-01-01T00:00:00.000Z']
        );
        await run(
            db,
            `INSERT INTO articoli (id, codice, cliente, commessa, lunghezza, posizione, fila, offset_inizio, tipo, livello, colore, stato, data_creazione)
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
            ['s2', 'S2', 'Cliente', 'Comm', 1, 'P4 @ Pila (L1)', 'P4', 0, 'SOLETTA', 1, '#222222', 'IN_AREA', '2026-01-02T00:00:00.000Z']
        );
        await run(
            db,
            `INSERT INTO articoli (id, codice, cliente, commessa, lunghezza, posizione, fila, offset_inizio, tipo, livello, colore, stato, data_creazione)
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
            ['s3', 'S3', 'Cliente', 'Comm', 1, 'P4 @ Pila (L2)', 'P4', 0, 'SOLETTA', 2, '#333333', 'IN_AREA', '2026-01-03T00:00:00.000Z']
        );
...
async function runApiIntegrationTests() {
    ...
    logPass('article creation enforces unique codes');
